...
@handle_exceptions
def list_files_and_directories(source_directory_path: Path, destination_directory_path: Path, depth: int = 0) -> None:
    """
    ...
    for path in source_directory_path.iterdir():
        ...
        if path.is_file():
            ...
            copy_file_to_destination(path, destination_directory_path)

        elif path.is_dir():
            print(f"{indent}{Fore.BLUE} {path.name}/")
            list_files_and_directories(path, destination_directory_path, depth + 1)

@handle_exceptions
def copy_file_to_destination(path: Path, destination_directory_path: Path) -> None:
    """
    ...
    if file_extension:
        ...
        shutil.copy2(path, new_dir / path.name)
